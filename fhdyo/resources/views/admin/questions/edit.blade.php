...
<div class="max-w-2xl mx-auto">
    ...
    <div class="mb-8">
        ...
        <h2 class="text-2xl sm:text-3xl font-bold text-slate-900">Savolni Tahrirlash</h2>
        <p class="text-slate-500 mt-1">Savol ma'lumotlarini yangilang</p>
    </div>

    <!-- Form - Soft UI -->
    <form action="{{ route('admin.questions.update', $question) }}" method="POST" class="bg-white rounded-2xl p-6 sm:p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
        @csrf
        @method('PUT')
        ...
        <div class="mb-6">
            ...
            >
                ...
                @foreach($units as $unit)
                    <option value="{{ $unit->id }}" {{ old('unit_id', $question->unit_id) == $unit->id ? 'selected' : '' }}>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <label class="relative flex items-center p-4 bg-slate-50 border-2 border-slate-200 rounded-xl cursor-pointer hover:bg-slate-100 hover:border-slate-300 transition-all has-[:checked]:border-indigo-500 has-[:checked]:bg-indigo-50">
                    <input
                        ...
                        name="is_critical"
                        value="1"
                        {{ old('is_critical', $question->is_critical) === '1' ? 'checked' : '' }}
                ...
                <label class="relative flex items-center p-4 bg-slate-50 border-2 border-slate-200 rounded-xl cursor-pointer hover:bg-slate-100 hover:border-slate-300 transition-all has-[:checked]:border-indigo-500 has-[:checked]:bg-indigo-50">
                    <input
                        ...
                        name="is_critical"
                        value="0"
                        {{ old('is_critical', $question->is_critical) === '0' ? 'checked' : '' }}
                        class="w-4 h-4 text-indigo-600 bg-slate-100 border-slate-300 focus:ring-indigo-500"
                    >
                    <div class="ml-3">
