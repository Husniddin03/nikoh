...
<div class="max-w-2xl mx-auto">
    ...
    <div class="mb-8">
        ...
        <h2 class="text-2xl sm:text-3xl font-bold text-slate-900">Bo'limni Tahrirlash</h2>
        <p class="text-slate-500 mt-1"><span class="inline-flex items-center px-2 py-0.5 bg-slate-100 rounded-lg text-sm font-medium text-slate-700 mr-1">{{ $unit->name }}</span> bo'limini yangilang</p>
    </div>

    <!-- Form - Soft UI -->
    <form action="{{ route('admin.units.update', $unit) }}" method="POST" class="bg-white rounded-2xl p-6 sm:p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
        @csrf
        @method('PUT')
        ...
        <div class="mb-6">
            ...
            >
                ...
                <option value="nikoh" {{ old('category', $unit->category) === 'nikoh' ? 'selected' : '' }}>Nikoh</option>
                <option value="ajrim" {{ old('category', $unit->category) === 'ajrim' ? 'selected' : '' }}>Ajrim</option>
            </select>
            @error('category')
                <p class="mt-2 text-sm text-rose-600 flex items-center gap-1">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
